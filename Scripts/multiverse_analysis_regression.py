...
output_path = PROJECT_ROOT / 'output_check' / 'age'
...
sys.path.insert(1, '/Users/amnesia/PycharmProjects/into-the-multiverse_fin')
# sys.path.append('/Users/amnesia/PycharmProjects/into-the-multiverse_fin')
import pickle
import random
...
with tqdm(range(n_thr * n_pre * n_BCT)) as pbar:
    for pre_idx, DataPreproc in enumerate(preprocessings):  # data preprocessing
        if DataPreproc == 'MRS':
            TempData = MainNoNanModelSpace                                              # Functional connectivity: 346x346x50 - ROIxROIxSub
...
# pickle.dump(data_reduced, open(str(output_path / "embeddings_2.p"), "wb" ) )
...
ModelEmbeddings = pickle.load(open(str(output_path / "embeddings_2.p"), "rb"))
# todo find why model embeddings are missing PCA
ModelEmbedding = ModelEmbeddings['SE']
...
pickle.dump(PredictedAcc, open(str(output_path / 'predictedAcc_SE.pckl'), 'wb'))
...
corr, nuModEmb2 = plot_bo_evolution(kappa, x_exploratory, y_exploratory, z_exploratory, x, y, gp,
